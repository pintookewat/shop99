class ProductsController < ApplicationController
  ...
  def create
    ...
    if product.update(product_id: product_id[:id], price_id: product_price[:id], url: product_url[:url])
      ...
      #   binding.break
      ProductmadeMailer.with(product: product).new_product_email.deliver_now
    else
  ...
  def product_params
    params.require(:product).permit(:product_id, :default_price, :description, :images, :price, :name, :payment_id,
                                    :object, :url, :price_id, :user_id)
  end
end
